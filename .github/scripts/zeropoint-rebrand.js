/**
 * lorenzkahl.de Template Rebranding Script
...
async function rebrand() {
  ...
  // Safety check - ensure we're working with a lorenzkahl.de template
  const packageJson = JSON.parse(readFileSync('package.json', 'utf8'));
  if (packageJson.name !== 'lorenzkahl.de') {
    // Clean up template marker if this isn't a lorenzkahl.de template
    unlinkSync('.template');
    if (isCI) {
      console.log('⚠️ Not a lorenzkahl.de template, skipping...');
    }
    process.exit(0);
  }

  console.log('🪐 Welcome to lorenzkahl.de! Let\'s customize this for your project.\n');
  ...
  replaceInFiles('lorenzkahl.de', projectName.replace(/\s+/g, '')); // Remove spaces from project name
  replaceInFiles('lorenzkahl', author); // Replace default author
  replaceInFiles('lorenzkahl.de', projectName.toLowerCase().replace(/\s+/g, '-')); // Lowercase with hyphens
  ...
  if (existsSync('README.lorenzkahl.de.md')) {
    try {
      copyFileSync('README.lorenzkahl.de.md', 'README.md'); // Use lorenzkahl.de README as new README
    } catch (error) {
      console.warn(`Could not copy README.lorenzkahl.de.md: ${error.message}`);
    }
  }
